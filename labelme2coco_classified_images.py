...
class labelme2coco(object):
    ...
    def image(self, data, num, json_path):
        ...
        image["width"] = width
        image["id"] = num
        image["file_name"] = os.path.join(os.path.dirname(json_path),data["imagePath"].split("/")[-1])
...
def find_all_json_file(filepath):
    all_files = os.listdir(filepath)
    for f in all_files:
        newdir = os.path.join(filepath, f)  # 将文件名加入到当前文件路径后面
        if os.path.isfile(newdir):  # 如果是文件
            if os.path.splitext(newdir)[1] == ".json":  # 如果文件是".json"后缀的
                labelme_json.append(newdir)
        elif os.path.isdir(newdir):  # 如果是路径
            find_all_json_file(newdir)  # 递归
...
if __name__ == "__main__":
    ...
    find_all_json_file(args.labelme_images)
